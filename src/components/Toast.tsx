...
function ToastItem({
  ...
  return <motion.div layout initial={{
    ...
    scale: 0.9
      ...
      {toast.duration && toast.duration > 0 && <motion.div initial={{
      width: '100%'
    }} animate={{
      width: '0%'
    }} transition={{
      ...
      ease: 'linear'
    }} className={`absolute bottom-0 left-0 h-1 ${toast.type === 'success' ? 'bg-green-500' : toast.type === 'error' ? 'bg-red-500' : toast.type === 'warning' ? 'bg-yellow-500' : 'bg-blue-500'}`} />}
    </motion.div>;
}